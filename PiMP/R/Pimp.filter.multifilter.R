Pimp.filter.multifilter <- function(file=NULL, mzmatch.filters=list(), mzmatch.params=list(), mzmatch.outputs=list()) {
	...
	{
		mzmatch.ipeak.filter.NoiseFilter(i=file, o=mzmatch.outputs$final.combined.noise.filtered.file, v=T, codadw=mzmatch.params$noise)
	...
	if(length(values)>0) {
		...
		{
			cat(paste("mzmatch.ipeak.filter.SimpleFilter(i=\"", file, "\",o=\"", mzmatch.outputs$final.combined.simple.filtered.file,"\",v=T,", paste(names(mzmatch.params[values]), mzmatch.params[values], sep="=", collapse=","), ")", sep=""))
			eval(parse(text=paste("mzmatch.ipeak.filter.SimpleFilter(i=\"", file, "\",o=\"", mzmatch.outputs$final.combined.simple.filtered.file,"\",v=T,", paste(names(mzmatch.params[values]), mzmatch.params[values], sep="=", collapse=","), ")", sep="")))
