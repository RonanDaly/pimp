...
def start_pimp_pipeline(analysis, project, user):
	...
	xml_file_path = os.path.join(os.path.dirname(settings.BASE_DIR), 'pimp_data', 'projects', str(project.id), xml_file_name)
	...
	if os.path.exists(xml_file_path):
		...
		for peak_element in xmltree.getPeaks():
			...
			peak = Peak(dataset=dataset, secondaryId=peakSecondaryId, mass=peakInfo["mass"], rt=peakInfo["retention_time"], polarity=peakInfo["polarity"], type=peakInfo["peak_type"])
			peak.save()
			# print peak.id
			######## Debug ########
			...
			if comparisons:
				for comparison_element in comparisons:
					...
					peak_dt_comparison = PeakDtComparison(peak=peak, comparison=comparison, logFC=comparisonInfo["logfc"], pValue=comparisonInfo["pvalue"], adjPvalue=comparisonInfo["adjpvalue"], logOdds=comparisonInfo["logodds"])
			...
			if calibrationReferences:
				for calibrationReference_element in calibrationReferences:
					...
					# print sample.name
					calibrationReferenceIntensity = xmltree.getCalibrationReferenceIntensityFromElement(calibrationReference_element)
					peak_qc_sample = PeakQCSample(peak=peak, sample=calibrationSample, intensity=calibrationReferenceIntensity)
					peak_qc_sample.save()
