...
def get_identification_table(request, project_id, analysis_id):
    if request.is_ajax():
        ...
        data = [[str(compound.secondaryId), str(compound.id), str(compound.peak.secondaryId), " ".join(list(set(
            [x.lower() for x in compound.repositorycompound_set.values_list('compound_name', flat=True).distinct()]))),
                 str(compound.formula), str(compound.ppm), str(round(compound.peak.rt, 2)),
                 "Identified" if compound.identified == "True" else "Annotated", str(compound.peak.polarity)] for
