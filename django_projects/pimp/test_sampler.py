	...
	for p in list_of_peaks:
		...
		for a in ca:
			...
			if len(previous_pos) == 0:
				...
				newmeasurement.annotations[peakset.annotations[-1]] = a.confidence
			else:
				...
				if this_annotation in newmeasurement.annotations:
					...
					newmeasurement.annotations[this_annotation] = max(a.confidence,current_confidence)
				else:
					newmeasurement.annotations[this_annotation] = a.confidence
