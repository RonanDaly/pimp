...
folder are mapped to the application's root namespace, 'metExploreViz'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "metExploreViz"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'metExploreViz' in the same way
as `"metExploreViz/sass/src"`.
